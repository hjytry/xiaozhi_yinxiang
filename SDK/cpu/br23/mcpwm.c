...
u8 auto_reduce_bright_flag = FALSE;
u16 auto_reduce_bright_timer = 0;
...
void mcpwm_init(struct pwm_platform_data *arg)
{
    ...
    if (arg->h_pin == pwm_hw_h_pin[arg->pwm_ch_num]) {    //硬件引脚
        ...
        if (arg->h_pin < IO_MAX_NUM) {    //任意引脚
            ...
            gpio_output_channle(IO_PORTC_05, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTC_04, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTC_03, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTC_02, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTA_10, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTA_09, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTA_01, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_output_channle(IO_PORTA_00, CHx_CHx_PWM_H[arg->h_pin_output_ch_num][arg->pwm_ch_num]);
            gpio_set_hd0(IO_PORTC_05,1);
            gpio_set_hd(IO_PORTC_05,1);

            gpio_set_hd0(IO_PORTC_04,1);
            gpio_set_hd(IO_PORTC_04,1);

            gpio_set_hd0(IO_PORTC_03,1);
            gpio_set_hd(IO_PORTC_03,1);

            gpio_set_hd0(IO_PORTC_02,1);
            gpio_set_hd(IO_PORTC_02,1);

            gpio_set_hd0(IO_PORTA_10,1);
            gpio_set_hd(IO_PORTA_10,1);

            gpio_set_hd0(IO_PORTA_09,1);
            gpio_set_hd(IO_PORTA_09,1);
...
void  led8_pwm_run(u8 pin,u8 brightness)
{
    ...
    pwm_p_data.duty = brightness*100;              //占空比50%
    //hw
    pwm_p_data.h_pin = pin;                   //没有则填 -1。h_pin_output_ch_num无效，可不配置
...
void user_printf_portA(u8 bitnum)
{
        r_printf("portA%d:DIR-%d,PU-%d,PD-%d,DIE-%d,OUT-%d,IN-%d\n",
            bitnum,
            (JL_PORTA->DIR&BIT(bitnum))?1:0,
            (JL_PORTA->PU &BIT(bitnum))?1:0,
            (JL_PORTA->PD &BIT(bitnum))?1:0,
            (JL_PORTA->DIE&BIT(bitnum))?1:0,
            (JL_PORTA->OUT&BIT(bitnum))?1:0,
            (JL_PORTA->IN &BIT(bitnum))?1:0
        );
}

void user_printf_portC(u8 bitnum)
{
        r_printf("portC%d:DIR-%d,PU-%d,PD-%d,DIE-%d,OUT-%d,IN-%d\n",
            bitnum,
            (JL_PORTC->DIR&BIT(bitnum))?1:0,
            (JL_PORTC->PU &BIT(bitnum))?1:0,
            (JL_PORTC->PD &BIT(bitnum))?1:0,
            (JL_PORTC->DIE&BIT(bitnum))?1:0,
            (JL_PORTC->OUT&BIT(bitnum))?1:0,
            (JL_PORTC->IN &BIT(bitnum))?1:0
        );
}

void led8_pwm_stop(void)
{
    mcpwm_close(pwm_ch2,pwm_timer2);
    // while(1)
    // {
    //     user_printf_portC(5);
    //     user_printf_portC(4);
...
u8 bright_arr[] = {0,30,60,100};
u8 bright_index = 3;
static u8 bright_open_flag = FALSE;
u8 bright_cur_index = 3;
extern void  user_send_brightness_state(void);
void led8_set_pwm_duty(u8 duty,bool en)
{
    r_printf("%s %d %d\n",__func__,duty,en);
    u8 i = 0;
    duty = duty>100?100:duty;
    mcpwm_set_duty(pwm_ch2, pwm_timer4, duty*100);
    for(i = 0;i < sizeof(bright_arr);i++)
    {
        if(duty <= bright_arr[i])
        {
            bright_cur_index = i;
            //syscfg_write(CFG_USER_BRIGHTNESS,&bright_cur_index,1);
            break;
        }
    }
#if (SMART_BOX_EN)
    if(en){
        user_send_brightness_state();
    }
#endif
}


void pwm_to_adjust_bright(void)
{
    bright_open_flag = TRUE;
    bright_index = (bright_index>=(sizeof(bright_arr)-1))?0:(bright_index+1);
    led8_set_pwm_duty(bright_arr[bright_index],1);
    r_printf("bright_index == %d\n",bright_index);
} 
void pwm_to_adjust_up_bright(void)
{
    auto_reduce_bright_flag = FALSE;
    bright_open_flag = TRUE;
    bright_index = sizeof(bright_arr)-1;
    led8_set_pwm_duty(bright_arr[bright_index],1);
    r_printf("bright_index == %d\n",bright_index);
} 

void pwm_to_adjust_down_bright(void)
{
    auto_reduce_bright_flag = FALSE;
    bright_open_flag = TRUE;
    bright_index = 1;
...
void pwm_to_auto_reduce_bright(void)
{
    if((bright_index >= (sizeof(bright_arr)-2))&&(!auto_reduce_bright_flag))
    {
        r_printf("pwm_to_auto_reduce_bright\n");
        auto_reduce_bright_flag = TRUE;
        led8_set_pwm_duty(bright_arr[1],1);
    }
}

void pwm_to_recovery_bright(void)
{
    r_printf("app_get_curr_task %d\n",app_get_curr_task());
    if(APP_IDLE_TASK != app_get_curr_task()){
        if((((auto_reduce_bright_flag)&&(bright_index >= (sizeof(bright_arr)-2)))&&(auto_reduce_bright_timer))||(!bright_open_flag))
        {
            r_printf("pwm_to_recovery_bright\n");
            auto_reduce_bright_flag = FALSE;
            bright_open_flag = TRUE;
            led8_set_pwm_duty(bright_arr[bright_index],1);
        }
...
void pwm_to_close_bright(void)
{
    bright_open_flag = FALSE;
    led8_set_pwm_duty(0,1);
}
void pwm_to_open_bright(void)
{
    bright_open_flag = TRUE;
    auto_reduce_bright_flag = FALSE;
    if(bright_index != 0)
...
void  led8_timer_start(void)
{
    // syscfg_read(CFG_USER_BRIGHTNESS,&bright_cur_index,1);
    // if((bright_cur_index < 0)||(bright_cur_index > (sizeof(bright_arr)-1))){
    //     bright_cur_index = 3;
    //     syscfg_write(CFG_USER_BRIGHTNESS, &bright_cur_index, 1);
    // }else if(bright_cur_index == 0){
    //     bright_cur_index = 1;
    //     syscfg_write(CFG_USER_BRIGHTNESS, &bright_cur_index, 1);
    // }
    // bright_index = bright_cur_index;
    bright_open_flag = TRUE;
    led8_pwm_run(IO_PORTC_05,bright_arr[bright_index]);
    auto_reduce_bright_timer = sys_timer_add(NULL, pwm_to_auto_reduce_bright, (120 * 1000));
}

void del_auto_reduce_bright_timer(void)
{
    if(auto_reduce_bright_timer)
    {
        sys_timer_del(auto_reduce_bright_timer);
        auto_reduce_bright_timer = 0;
    }
}

void recovery_reduce_bright_timer(void)
{
    if(auto_reduce_bright_timer){
       sys_timer_modify(auto_reduce_bright_timer, (60 * 1000));
    }
}
