...
void led7_clear_show_icon(UI_LED7_ICON icon)
{
    __this->led7_var.bShowIcon &= (~icon);
}

void led7_clear_flash_icon(UI_LED7_ICON icon)
{
    __this->led7_var.bFlashIcon &= (~icon);
}
...
void led7_clear_all_icon(void)
{
    ...
    __this->char_flash_time = jiffies_to_msecs(jiffies);
}
...
static u8 __ui_led7_char_get_sys_halfsec(void)
{
    if (((jiffies_to_msecs(jiffies) - __this->char_flash_time) & 999) > 500) {
        return true;
    } else {
        return false;
    }
...
static u8 __ui_led7_icon_get_sys_halfsec(void)
{
    if (((jiffies_to_msecs(jiffies) - __this->icon_flash_time) & 999) > 500) {
        return true;
    } else {
        return false;
    }
...
static void __ui_led7_update_bShowbuf1(void)
{
    ...
    //char flash
    if (__ui_led7_char_get_sys_halfsec()) { //500ms display, or char off
    ...
    if (__this->led7_var.bFlashIcon) {
        if (__ui_led7_icon_get_sys_halfsec()) { //500ms display, or seg off
            for (j = 0; j < 32; j++) {
                if (BIT(j) & __this->led7_var.bFlashIcon) {
                    for (i = 0; i < ARRAY_SIZE(led7_icon_seg2pin); i++) { //lookup icon exist
                        if (BIT(j) == led7_icon_seg2pin[i].icon) {
...
void led7_scan(void *param)
{
    ...
    extern void close_gpio_to_pwm(u32 gpio);
    extern void open_gpio_to_pwm(u32 gpio);
    if (__this->user_data->pin_type == LED7_PIN7) {
        //pin cnt output H
        gpio_set_direction(__this->user_data->pin_cfg.pin7.pin[cnt], 0);
        open_gpio_to_pwm(__this->user_data->pin_cfg.pin7.pin[cnt]);
...
static LCD_API LED7_HW = {
    ...
    .lock              = led7_show_lock,
    .clear_flash_icon  = led7_clear_flash_icon,
    .clear_show_icon   = led7_clear_show_icon
};
...
void add_led7_scan_to_timer(void)
{
    void app_timer_led_scan(void (*led_scan)(void *));
    app_timer_led_scan(led7_scan);    
}
