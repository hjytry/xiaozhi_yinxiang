...
/* static const char *watch_res[40] = { */
/* RES_PATH"watch/watch.sty", */
/* RES_PATH"watch1/watch1.sty", */
/* RES_PATH"watch2/watch2.sty", */
/* RES_PATH"watch3/watch3.sty", */
...
int watch_del_item(char *s)
{
    ...
    for (i = 0; i < watch_items; i++) {
        /* if (strncmp(cur_watch, watch_res[i], strlen(cur_watch)) == 0) { */
        /* break; */
        /* } */
...
int watch_set_init()
{
    ...
    memset(watch_res, 0, sizeof(watch_res));
    watch_style = 1;
    watch_items = sizeof(WATCH_STY_CHECK_LIST) / sizeof(WATCH_STY_CHECK_LIST[0]);

    for (i = 0; i < watch_items; i++) {
        len = strlen(WATCH_STY_CHECK_LIST[i]);
        watch_res[i] = malloc(len + 1);
        if (watch_res[i] == NULL) {
            printf("\n\nmalloc watch list err\n\n");
            while (1);
        }
        strcpy(watch_res[i], WATCH_STY_CHECK_LIST[i]);
        watch_res[i][len] = '\0';
