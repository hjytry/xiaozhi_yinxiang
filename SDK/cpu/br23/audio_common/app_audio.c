...
#define DEFAULT_DIGTAL_VOLUME   11466//16384
...
void app_audio_set_volume(u8 state, s8 volume, u8 fade)
{
    ...
    case APP_AUDIO_STATE_WTONE:
        ...
        if(alarm_tone_vol_flag != ALARM_NUMBER_NULL)
        {
            extern int alarm_tone_vol_1;
            extern int alarm_tone_vol_2;
            if(alarm_tone_vol_flag == ALARM_NUMBER_1){
                app_var.wtone_volume = alarm_tone_vol_1;
                r_printf("alarm_tone_vol_1 = %d\n",alarm_tone_vol_1);
            }else if(alarm_tone_vol_flag == ALARM_NUMBER_2){
                app_var.wtone_volume = alarm_tone_vol_2;
                r_printf("alarm_tone_vol_2 = %d\n",alarm_tone_vol_2);
            }
        }
...
{
    syscfg_read(CFG_MUSIC_VOL, &app_var.music_volume, 1);
#if AUDIO_VOLUME_INIT_FIXED
    app_var.music_volume = AUDIO_VOLUME_INIT_FIXED_VALUE;
#else
    if((app_var.music_volume <= 0)||(app_var.music_volume > 30))
    {
        app_var.music_volume = AUDIO_VOLUME_INIT_DEFAULT;
    }    
#endif
    app_audio_set_volume(APP_AUDIO_STATE_MUSIC, app_var.music_volume, 1);
    extern int syscfg_write(u16 item_id, void *buf, u16 len);
    syscfg_write(CFG_MUSIC_VOL, &app_var.music_volume, 1);
}
...
void app_audio_output_init(void)
{
    ...
    if (len != sizeof(dac_trim) || dac_trim.left == 0 || dac_trim.right == 0) {
        extern int audio_dac_do_trim_overwrite(struct audio_dac_hdl *dac, struct audio_dac_trim *dac_trim, u8 fast_trim);
        audio_dac_do_trim_overwrite(&dac_hdl, &dac_trim, 0);
