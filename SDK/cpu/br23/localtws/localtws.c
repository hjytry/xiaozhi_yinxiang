...
const u16 LOCALTWS_MIXER_BUF_LEN = (128 * 2 * 2);	// mixer空间大小，循环mixer没有直通
#else
...
int localtws_bt_event_deal(struct bt_event *evt)
{
    ...
    case TWS_EVENT_CONNECTED:
        tws_api_sync_call_by_uuid('D', SYNC_CMD_BOX_INIT_EXIT_BT, 0);
...
static void localtws_rx_data(void *data, u16 len, bool rx)
{
    ...
    u32 value = 0;
    memcpy(&value, data, LOCALTWS_MEDIA_INFO_LEN);
    y_printf("localtws_rx_data:0x%x \n", value);
    g_localtws.media_value = value;
    if (value) {
        localtws_dec_event(TWS_EVENT_LOCAL_MEDIA_START, value);
    } else {
        localtws_dec_event(TWS_EVENT_LOCAL_MEDIA_STOP, 0);
    }
}
...
#if LOCALTWS_CHANGE_USE_BT_CMD
    tws_api_send_data_to_sibling((u8 *)&g_localtws.media_value, 4, TWS_FUNC_ID_LOCALTWS);
#endif /*LOCALTWS_CHANGE_USE_BT_CMD*/
...
#if LOCALTWS_CHANGE_USE_BT_CMD
    int value = 0;
    tws_api_send_data_to_sibling((u8 *)&value, 4, TWS_FUNC_ID_LOCALTWS);
