...
#if (SOUNDCARD_ENABLE)
#define SOUNDCARD_PC_STREAM_ENABLE			1
#else
#define SOUNDCARD_PC_STREAM_ENABLE			0
#endif

#ifdef CONFIG_MIXER_CYCLIC
#undef SOUNDCARD_PC_STREAM_ENABLE
#define SOUNDCARD_PC_STREAM_ENABLE			0
#endif
...
#if (SOUNDCARD_PC_STREAM_ENABLE)
    struct audio_mixer_ch frfl_mix_ch;	// 叠加句柄
    struct audio_mixer_ch rrrl_mix_ch;	// 叠加句柄
...
#if (SOUNDCARD_PC_STREAM_ENABLE)
extern struct audio_mixer soundcard_frfl_mixer;
extern struct audio_mixer soundcard_rrrl_mixer;
#endif/*SOUNDCARD_PC_STREAM_ENABLE*/
//////////////////////////////////////////////////////////////////////////////
...
#if(SOUNDCARD_PC_STREAM_ENABLE)
        /* audio_buf_sync_follow_rate(&dec->frfl_mix_ch.sync, in_rate, out_rate); */
        /* audio_buf_sync_follow_rate(&dec->rrrl_mix_ch.sync, in_rate, out_rate); */
#else
...
#endif
#if(SOUNDCARD_PC_STREAM_ENABLE)
    audio_mixer_ch_close(&uac_dec->frfl_mix_ch);
    audio_mixer_ch_close(&uac_dec->rrrl_mix_ch);
    if (uac_dec->sync) {
        stream_sync_close(&uac_dec->sync);
    }
#else
...
static void audio_pc_check_timer(void *priv)
{
    ...
    if (uac_speaker_stream_size(NULL) == 0) {
        ...
        if (cnt == 15) {
#if (SOUNDCARD_PC_STREAM_ENABLE)
            audio_mixer_ch_pause(&uac_dec->frfl_mix_ch, 1);
            audio_mixer_ch_pause(&uac_dec->rrrl_mix_ch, 1);
#else
        ...
        if (cnt > 14) {
#if (SOUNDCARD_PC_STREAM_ENABLE)
            audio_mixer_ch_pause(&uac_dec->frfl_mix_ch, 0);
            audio_mixer_ch_pause(&uac_dec->rrrl_mix_ch, 0);
#else
...
static int rrrl_mix_ch_prob_handler(struct audio_stream_entry *entry,  struct audio_data_frame *in)
{
#if (SOUNDCARD_PC_STREAM_ENABLE)
    struct uac_dec_hdl *dec = uac_dec;
    if (dec) {
        if (dec->rrrl_output_en == 0) {
            putchar('*');
...
static int uac_audio_start(void)
{
    ...
    audio_mixer_ch_open_head(&dec->frfl_mix_ch, &soundcard_frfl_mixer);
    /* audio_mixer_ch_set_sync(&dec->frfl_mix_ch, &info, 1, 1); */
    audio_mixer_ch_set_no_wait(&dec->frfl_mix_ch, 1, 10); // 超时自动丢数
    audio_mixer_ch_open_head(&dec->rrrl_mix_ch, &soundcard_rrrl_mixer);
    //audio_mixer_ch_set_sync(&dec->rrrl_mix_ch, &info, 1, 1);
    audio_mixer_ch_set_no_wait(&dec->rrrl_mix_ch, 1, 10); // 超时自动丢数

    struct stream_sync_info info = {0};
    info.i_sr = MIC_EFFECT_SAMPLERATE;
    info.o_sr = MIC_EFFECT_SAMPLERATE;
    ...
    entries[entry_cnt++] = &dec->frfl_mix_ch.entry;
#else
    entries[entry_cnt++] = &dec->mix_ch.entry;
#endif/*SOUNDCARD_PC_STREAM_ENABLE*/
#endif
...
#if (SOUNDCARD_PC_STREAM_ENABLE)
    dec->rrrl_mix_ch.entry.prob_handler = rrrl_mix_ch_prob_handler;
    audio_stream_add_entry(entries[entry_cnt - 2], &dec->rrrl_mix_ch.entry);
#endif
...
#endif
#if (SOUNDCARD_PC_STREAM_ENABLE)
    audio_mixer_ch_close(&dec->frfl_mix_ch);
    audio_mixer_ch_close(&dec->rrrl_mix_ch);
    if (dec->sync) {
