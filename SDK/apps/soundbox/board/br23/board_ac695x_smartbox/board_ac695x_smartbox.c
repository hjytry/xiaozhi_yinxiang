...
static last_enable = FALSE;
void user_sd_set_power(u8 enable)
{
    ...
    if(enable)
    {
        gpio_set_pull_up(USER_SD_POWER_IO, 0);
        gpio_set_pull_down(USER_SD_POWER_IO, 0);
        gpio_set_direction(USER_SD_POWER_IO, 0);
        gpio_set_hd(USER_SD_POWER_IO,1);
        gpio_set_hd0(USER_SD_POWER_IO,1);
        gpio_direction_output(USER_SD_POWER_IO,1);
    }else{
        #if (!GET_TEMPERATURE_ENABLE)&&(TCFG_APP_LINEIN_EN)
        gpio_set_pull_up(USER_SD_POWER_IO, 0);
        gpio_set_pull_down(USER_SD_POWER_IO, 0);
        gpio_set_direction(USER_SD_POWER_IO, 1);
        gpio_set_hd(USER_SD_POWER_IO,0);
        gpio_set_hd0(USER_SD_POWER_IO,0);
        gpio_direction_output(USER_SD_POWER_IO,0);
        #endif
    }
#endif
    last_enable = enable;
...
//	.detect_mode            = SD_CMD_DECT,
    ...
    .power                  = user_sd_set_power,//库函数，使用SDPG引脚。可以自行重写其他的SD卡电源控制函数，传入0：关电源。传入1，开电源。如果电源硬件已固定不可控，则该函数无效，可以填NULL
    /* .power                  = NULL, */
#else
	.detect_func            = sdmmc_cmd_detect,
    .power                  = user_sd_set_power,//cmd检测需要全程供电，建议用硬件固定电源。当然，可以自行写其他的SD卡电源控制函数，传入0：关电源。传入1，开电源。
    /* .power                  = sd_set_power, */
#endif
...
//第二组ADKEY
const struct adkey_platform_data adkey1_data = {
    .enable = TCFG_ADKEY_ENABLE,                              //AD按键使能
    .adkey_pin = TCFG_ADKEY1_PORT,                             //AD按键对应引脚
    .ad_channel = TCFG_ADKEY1_AD_CHANNEL,                      //AD通道值
    .extern_up_en = TCFG_ADKEY_EXTERN_UP_ENABLE,              //是否使用外接上拉电阻
    .ad_value = {                                             //根据电阻算出来的电压值
        TCFG_ADKEY1_VOLTAGE0,
        TCFG_ADKEY1_VOLTAGE1,
        TCFG_ADKEY1_VOLTAGE2,
        TCFG_ADKEY1_VOLTAGE3,
    },
    .key_value = {                                             //AD按键各个按键的键值
        TCFG_ADKEY1_VALUE0,
        TCFG_ADKEY1_VALUE1,
...
LED7_PLATFORM_DATA_BEGIN(led7_data)
    ...
    .pin_cfg.pin7.pin[4] = IO_PORTA_10,
    .pin_cfg.pin7.pin[5] = IO_PORTA_09,
    .pin_cfg.pin7.pin[6] = IO_PORTA_01,
    .pin_cfg.pin7.pin[7] = IO_PORTA_00,
#endif
...
void board_init()
{
    ...
    set_lighting_mode(LIGHTING_INIT);
#if TCFG_CHARGE_ENABLE
...
extern void set_gpio_r_to_pwm(u32 gpio);
extern extern void led8_pwm_stop(void);
extern void del_app_timer_led_scan(void);
extern void  led8_timer_start(void);
extern void add_led7_scan_to_timer(void);
void close_led7_ui(void)
{
    del_app_timer_led_scan();
    led8_pwm_stop();
...
void add_led7_ui(void)
{
    led8_timer_start();
    add_led7_scan_to_timer();
}
void board_set_soft_poweroff(void)
{
    printf("board_set_soft_poweroff\n");
    del_app_timer_led_scan();
    led8_pwm_stop();
    ...
    gpio_set_die(MODE_PIN, 0);
    gpio_set_dieh(MODE_PIN, 0);
    ...
    gpio_set_hd(MUTE_PIN,0);
    gpio_set_hd0(MUTE_PIN,0);

    gpio_set_pull_up(IO_PORTB_11, 0);
    gpio_set_pull_down(IO_PORTB_11, 0);
    gpio_set_direction(IO_PORTB_11, 1);
    gpio_set_hd(IO_PORTB_11,0);
    gpio_set_hd0(IO_PORTB_11,0);
