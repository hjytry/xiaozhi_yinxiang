...
#define SOUNDCARD_UPDATE_MIC_PARM_EN				0
...
#if (SOUNDCARD_UPDATE_MIC_PARM_EN)
const struct __mic_stream_parm mic_parm_pitch = {
    .mic_gain			= 0,
...
const struct __mic_stream_parm mic_parm_normal = {
    .mic_gain			= 0,
    .sample_rate 		= MIC_EFFECT_SAMPLERATE,//采样率
...
void soundcard_update_mic_parm(struct __mic_stream_parm *parm)
{
    ...
    audio_dig_vol_set(audio_dig_vol_group_hdl_get(sys_digvol_group, "music_linein"), AUDIO_DIG_VOL_ALL_CH, 0);
    audio_dig_vol_set(audio_dig_vol_group_hdl_get(sys_digvol_group, "mic_mic"), AUDIO_DIG_VOL_ALL_CH, 0);
    printf("1111\n");
    os_time_dly(20);
    printf("2222\n");
    ...
    audio_adc_set_irq_point_unit(parm->point_unit);
...
#endif/*SOUNDCARD_UPDATE_MIC_PARM_EN*/
...
{
#if (SOUNDCARD_UPDATE_MIC_PARM_EN)
    u16 cur_mode = mic_effect_get_cur_mode();
    u8 pitch = 0;
    if (cur_mode == EFFECT_REVERB_MODE_BOY_TO_GIRL
        || cur_mode == EFFECT_REVERB_MODE_GIRL_TO_BOY
        || cur_mode == EFFECT_REVERB_MODE_KIDS
        || cur_mode == EFFECT_REVERB_MODE_MAGIC
       ) {
        pitch = 1;
    }
    if (mode == EFFECT_REVERB_MODE_BOY_TO_GIRL
        || mode == EFFECT_REVERB_MODE_GIRL_TO_BOY
        || mode == EFFECT_REVERB_MODE_KIDS
        || mode == EFFECT_REVERB_MODE_MAGIC
       ) {
        printf("is pitch mode = %d\n", pitch);
        if (pitch == 0) {
            soundcard_update_mic_parm((struct __mic_stream_parm *)&mic_parm_pitch);
        }
        mic_effect_change_mode(mode);
    } else {
        printf("is not pitch mode = %d\n", pitch);
        mic_effect_change_mode(mode);
        if (pitch == 1) {
            soundcard_update_mic_parm((struct __mic_stream_parm *)&mic_parm_normal);
        }
    }
#else
    mic_effect_change_mode(mode);
