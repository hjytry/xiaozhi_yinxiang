...
static void rcsp_user_event_ble_handler(ble_state_e ble_status)
{
    switch (ble_status) {
    case BLE_ST_IDLE:
#if RCSP_UPDATE_EN
...
static void rcsp_user_event_spp_handler(u8 spp_status)
{
    switch (spp_status) {
    case 0:
        break;
...
void rcsp_user_event_handler(u16 opcode, u8 *data, int size)
{
    ...
    case BT_3TH_EVENT_COMMON_BLE_STATUS:
        ble_status = data[0];
        rcsp_user_event_ble_handler(ble_status);
        break;

    case BT_3TH_EVENT_COMMON_SPP_STATUS:
        spp_status = data[0];
        rcsp_user_event_spp_handler(spp_status);
        break;
