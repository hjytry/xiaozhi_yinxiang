...
enum {
    ...
    ALARM_IDEX_TONE_MAX_NUM = 6,
};
static const char *default_ringtone_table[] = {
    [ALARM_IDEX_TONE_NUM_0] 			= TONE_RES_ROOT_PATH"tone/alarm1.*",
    [ALARM_IDEX_TONE_NUM_1] 			= TONE_RES_ROOT_PATH"tone/alarm2.*",
    [ALARM_IDEX_TONE_NUM_2] 			= TONE_RES_ROOT_PATH"tone/alarm3.*",
    [ALARM_IDEX_TONE_NUM_3] 			= TONE_RES_ROOT_PATH"tone/alarm4.*",
    [ALARM_IDEX_TONE_NUM_4] 			= TONE_RES_ROOT_PATH"tone/alarm5.*",
...
static const char *default_ringtone_name_table[] = {
    [ALARM_IDEX_TONE_NUM_0] 			= "alarm1",
    [ALARM_IDEX_TONE_NUM_1] 			= "alarm2",
    [ALARM_IDEX_TONE_NUM_2] 			= "alarm3",
    [ALARM_IDEX_TONE_NUM_3] 			= "alarm4",
    [ALARM_IDEX_TONE_NUM_4] 			= "alarm5",
    // [ALARM_IDEX_TONE_NUM_5] 			= "5",
    // [ALARM_IDEX_TONE_NUM_6] 			= "6",
    // [ALARM_IDEX_TONE_NUM_7] 			= "7",
    // [ALARM_IDEX_TONE_NUM_8] 			= "8",
...
extern void user_alarm_delete(u8 index);
...
#if KEY_SLEEP_ENABLE
    if(tmp_alarm.index == 0){
        user_alarm_delete(3);
    }else if(tmp_alarm.index == 1){
        user_alarm_delete(4);
    }
#endif
...
static u8 smart_rtc_alarm_extra_data_set(u8 index, u8 *p, u8 len)
{
    ...
    offset += data.ring_name_len;
        if(index == 0){
        // alarm_ring_flag_1 = rtc_ring_index[data.clust];
    }else if(index == 1){
        // alarm_ring_flag_2 = rtc_ring_index[data.clust];        
    }
...
u32 rtc_func_get(void *priv, u8 *buf, u16 buf_size, u32 mask)
{
    ...
    if (mask & BIT(RTC_INFO_ATTR_RTC_ALRAM)) {
        printf("RTC_INFO_ATTR_RTC_ALRAM\n");
        u8 alarm_data[2 * (sizeof(T_ALARM_APP) + M_MAX_ALARM_NAME_LEN + sizeof(T_ALARM_APP_EXTRA_DATA))];
        u8 total = 0;
        u8 index = 0;
        u8 name_len = 0;
        u8 *pTmp;
