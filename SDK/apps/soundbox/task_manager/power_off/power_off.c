...
extern void led7_show_null(void); //清除所有显示(数字,字符串和图标)
void poweroff_done(void)
{
    pwm_to_close_bright();
    led7_show_null();
...
static u16 sw_light_of_led_timer = 0;
void sw_light_of_led(void)
{
    r_printf("sw_light_of_led\n");
    static u8 cnt = 0;
    static u8 light_flag = TRUE;
    if (!tone_get_status()) {
        if(cnt++ >= 2){
    ...
    if(light_flag){
        UI_SHOW_MENU(MENU_POWER_UP, 150, 0, NULL);
    }else{
        UI_SHOW_MENU(MENU_CLEAR, 150, 0, NULL);        
    }

    light_flag = !light_flag;
    sw_light_of_led_timer = sys_timeout_add(NULL,sw_light_of_led,150);
...
static void poweroff_app_start()
{
    ...
    if (app_var.goto_poweroff_flag) {
        ...
        UI_SHOW_MENU(MENU_POWER_OFF, 2000, 0, NULL);
        // sw_light_of_led();
...
#endif/*CONFIG_TWS_POWEROFF_SAME_TIME*/
        {
            extern void poweroff_tone_play_start(void);
            // poweroff_tone_play_start();
