...
static int music_key_event_opr(struct sys_event *event)
{
    ...
    case KEY_MUSIC_CHANGE_REPEAT:
        ...
        break;
    case KEY_MUSIC_CHANGE_REPEAT_ONE_ALL:
        log_i("KEY_MUSIC_CHANGE_REPEAT_ONE_ALL\n");
        extern int music_player_change_one_all_mode(void);
        mode = music_player_change_one_all_mode();
        if (mode > 0) {
            UI_SHOW_MENU(MENU_MUSIC_REPEATMODE, 1000, mode, NULL);
        }
    break;
...
void app_music_task()
{
    ...
    //UI_SHOW_MENU(MENU_FILE_NUMBER, music_player_get_file_cur(), 0, NULL);
#if (MUSIC_DEVICE_TONE_EN)
    music_player_play_start();
#else
