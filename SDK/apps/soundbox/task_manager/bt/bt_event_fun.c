...
*/
/*----------------------------------------------------------------------------*/
extern void bule_no_connect_to_rtc(void *priv);
void sys_auto_shut_down_enable(void)
{
    ...
    if (app_var.auto_shut_down_timer == 0) {
        ...
        u8 app = app_get_curr_task();
        if (app == APP_BT_TASK) {
            app_var.auto_shut_down_timer = sys_timeout_add(NULL, bule_no_connect_to_rtc, (app_var.auto_off_time * 1000));
        } else {
...
void bt_status_phone_income(struct bt_event *bt)
{
    ...
    a2dp_dec_close();/*此处修复问题: income后a2dp未挂起但蓝牙已断流导致音频断流*/
    /*
