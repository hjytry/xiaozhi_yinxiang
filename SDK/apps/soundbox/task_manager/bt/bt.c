...
void bt_function_select_init()
{
    ...
    */
    ...
    {
        u8 tmp_ble_addr[6];
        // u8 xiaoyun_addr[6] = {0x8C,0xB4,0x58,0xAD,0x0B,0xCF};
        // bt_update_mac_addr(xiaoyun_addr);
...
static u16 get_phone_time_post_timer = 0;
void get_phone_time_post(void)
{
...
#if (SMART_BOX_EN)
            extern void  recovery_network_fm_state(void);
            recovery_network_fm_state();
#endif
    }else{
        if(get_phone_time_post_timer){
            sys_timeout_del(get_phone_time_post_timer);
            get_phone_time_post_timer = 0;
        }
        get_phone_time_post_timer = sys_timeout_add(NULL,get_phone_time_post,500);
    }
...
*/
/*----------------------------------------------------------------------------*/
extern void set_bt_ldotrim(u8 mode,u16 vm_id);
static int bt_connction_status_event_handler(struct bt_event *bt)
{
...
*/
/*----------------------------------------------------------------------------*/
extern bool bt_back_mode_falg;
int bt_background_event_handler_filter(struct sys_event *event)
{
    u8 ret = 0;
// #if TCFG_BLUETOOTH_BACK_MODE
    if(bt_back_mode_falg){
...
/*----------------------------------------------------------------------------*/
u8 map_test[10];
extern bool user_poweroff_cnt_flag;
int bt_key_event_handler(struct sys_event *event)
{
    int ret = true;
...
u8 check_and_to_number(u16 tenf,u16 singled){
    u16 temp = 0;

    if(((tenf<'0')&&(tenf>'9'))||((singled<'0')&&(singled>'9'))){
        return 0xFFFF;
    }else{
        temp = tenf-'0';
        temp = temp*10 +singled-'0';
        return temp;
    }
}
extern void get_rtc_sys_time(struct sys_time *time);
extern u8 user_week_flag;
void bluetooth_sync_time(struct sys_time *time)
{
...
void __attribute__ ((weak)) phone_date_and_time_feedback(u8 *data, u16 len)
{
    u16 year    = 0;
    u16 month   = 0;
    u16 day     = 0;
    ...
    r_printf("%s\n",__func__);
    year    = check_and_to_number(data[0],data[1]);
    month   = check_and_to_number(data[3],data[4]);
    day     = check_and_to_number(data[6],data[7]);
    hour    = check_and_to_number(data[10],data[11]);
    min     = check_and_to_number(data[13],data[14]);
    sec     = check_and_to_number(data[16],data[17]);

    if((hour == 0xFFFF)||(min == 0xFFFF)||(sec == 0xFFFF)||(year == 0xFFFF)||(month == 0xFFFF)||(day == 0xFFFF)){
        r_printf("phone time err\n");
    }else{
        year += 2000;
        r_printf("hour = %d,min = %d,sec = %d\n",hour,min,sec);
        get_rtc_sys_time((u32*)&current_time);
        ...
        bluetooth_sync_time(&current_time);
    }
    put_buf(data,len);
    puts(data);
...
void app_bt_task()
{
    ...
    // if ((!__this->cmd_flag)&&(!alarm_active_flag_get())) { //蓝牙后台拉回蓝牙模式不播放提示音
    if ((!alarm_active_flag_get())) { //蓝牙后台拉回蓝牙模式不播放提示音
