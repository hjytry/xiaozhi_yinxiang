...
T_ALARM alarm_tab[M_MAX_ALARM_NUMS] = {0};
...
void user_get_sys_time(struct sys_time *time)//获取时间
{
    get_sys_time(time);
}
...
void alarm_hw_write_time(struct sys_time *time, u8 sw)//写闹钟寄存器
{
    ...
    alarm_printf("xxx alarm_time : %d-%d-%d,%d:%d:%d\n", time->year, time->month, time->day, time->hour, time->min, time->sec);
...
//获取闹钟所有信息
void alarm_vm_read_info(PT_ALARM_MAP map)
{
    PRINT_FUN();
    T_ALARM_VM tmp;
    int ret = 0;
    ...
    for (i = 0; i < M_MAX_ALARM_NUMS; i++) {
        ...
        r_printf("vm info : index=%d, sw=%d, mode=%d,y=%d,m=%d,d=%d, h=%d, m=%d, name_len=%d\n", tmp.alarm.index, tmp.alarm.sw, tmp.alarm.mode, \
                tmp.alarm.time.year,tmp.alarm.time.month,tmp.alarm.time.day ,\
...
static void alarm_vm_write_info_by_index(PT_ALARM_MAP map, u8 index)
{
    ...
    alarm_printf("vm info : index=%d, sw=%d, mode=%d,y=%d,m=%d,d=%d, h=%d, m=%d, name_len=%d\n", tmp.alarm.index, tmp.alarm.sw, tmp.alarm.mode, \
                tmp.alarm.time.year,tmp.alarm.time.month,tmp.alarm.time.day ,\
                tmp.alarm.time.hour, tmp.alarm.time.min, tmp.alarm.name_len);
...
static int user_alarm_cmp_time(struct sys_time *time1, struct sys_time *time2,int *timer_difference)
{
    u8 i;
    int ret = 0;
    ...
    r_printf("time2 = %d,time1 = %d\n",((time2->hour*60)+time2->min),((time1->hour*60)+time1->min));
    *timer_difference =  ((time2->hour*60)+time2->min) - ((time1->hour*60)+time1->min);
    return ret;
}
...
static void alarm_calc_real_time_by_index(struct sys_time *cTime, u8 index)
{
    ...
    u32 c_tmp = ((cTime->hour & 0x1f) << 12) | ((cTime->min & 0x3f) << 6);
    u32 p_tmp = ((pTime->hour & 0x1f) << 12) | ((pTime->min & 0x3f) << 6);
    r_printf("c_tmp ==%d,p_tmp == %d\n",c_tmp,p_tmp);
    if (p_tmp > c_tmp) { //时间未到
...
static void __alarm_get_the_earliest(void)
{
    ...
    r_printf("find the %dth alarm, the save alarm : %x\n", index, alarm_map.active_map);
    alarm_hw_write_time(pTmp, alarm_tab[index].sw);
    // if((index == 1)||(index == 3))
    // {
    //     u8 i = 1;
    //     syscfg_write(CFG_USER_ALARM_NUMBER,&i,1);
...
u8 user_alarm_get_info(PT_ALARM p, u8 index)
{
    index--;
    return alarm_get_info(p,index);
}
...
{
    u8 active_alarm_number = 0;
    u8 active_alarm_index[4] = {0};
    PRINT_FUN();
    ...
    get_sys_time(&time);
    u8 i = 0;
    r_printf("alarm_map.active_map =%x\n", alarm_map.active_map);
    local_irq_disable();
    alarm_cur_active = alarm_map.active_map;
    if((alarm_map.active_map & BIT(1))||(alarm_map.active_map & BIT(2))||(alarm_map.active_map & BIT(3))||(alarm_map.active_map & BIT(4)))
    {
    ...
    for (i = 0; i < M_MAX_ALARM_NUMS; i++) {
        if (alarm_map.active_map & BIT(i)) {
            active_alarm_number++;
            active_alarm_index[active_alarm_number-1] = i;
            ...
            g_printf("alarm_number == %d\n",i);
        }
    }
    if((active_alarm_number > 0)&&(active_alarm_number <= 1)){
        if((alarm_map.active_map == BIT(0))||(alarm_map.active_map == BIT(2))){
            alarm_number_flag = ALARM_NUMBER_1;
        }else{
            alarm_number_flag = ALARM_NUMBER_2;
        }
    }else if(active_alarm_number >= 2){
        if((alarm_map.active_map & BIT(1))&&(!(alarm_map.active_map & BIT(0)))){
            alarm_number_flag = ALARM_NUMBER_2;
        }else{
            alarm_number_flag = ALARM_NUMBER_1;
        }
...
u8 user_alarm_add(PT_ALARM p, u8 index)
{
    index--;
    p->index--;
    return alarm_add(p,index);
}

void user_alarm_delete(u8 index)
{
    index--;
    alarm_delete(index);
}
...
void app_alarm_del_all(void)
{
    app_alarm_del(1);
    app_alarm_del(2);   
}

void app_alarm_add(u8 type,struct sys_time *alarm_time)
{
    ...
    int alarm1_time = 0;
    u8 push_index = 0;
    if((E_FAILURE == user_alarm_get_info(&alarm, 1))||(!alarm.sw))
    {
        alarm_writer_index = 1;
    }else if(E_FAILURE == user_alarm_get_info(&alarm, 2)||(!alarm.sw))
    {
        alarm_writer_index = 2;
    }

    if(alarm_writer_index==0)
    {
        syscfg_read(CFG_USER_PUSH_ALARM_ORDER,&push_index,1);
        if((push_index < 1)||(push_index >2))
        {
            alarm_writer_index = 1;
            syscfg_write(CFG_USER_PUSH_ALARM_ORDER,&alarm_writer_index,1);
        }else{
            alarm_writer_index = (push_index == 1)?2:1;
            syscfg_write(CFG_USER_PUSH_ALARM_ORDER,&alarm_writer_index,1);
        }
    }
...
u8 alarm_active_flag_get(void)
{
    r_printf("alarm_active_flag_get = %d\n",g_alarm_active_flag);
...
void get_alarm_vm_data(void)
{
    alarm_vm_read_info(&alarm_map);
}
u8 alarm_power_on_flag = FALSE;
extern bool reset_of_normal_flag;
void alarm_init()
{
    r_printf("alarm_init\n");
    ...
    alarm_vm_read_info(&alarm_map);
#if USER_POWEROFF==0
    sys_timer_add(NULL,get_alarm_vm_data,20000);
#endif
...
extern u8 user_alarm_set_vm_num;
    ...
    extern void set_rtc_info_of_mode(u8 cmd);
    r_printf("reset_of_normal_flag == %d\n",reset_of_normal_flag);
    if(reset_of_normal_flag){
        reset_of_normal_flag = FALSE;
        user_alarm_get_info(&temp_alarm, 1);
        temp_alarm.sw = 0;
        temp_alarm.index = 1;
        user_alarm_delete(temp_alarm.index);
        temp_alarm.mode  = E_ALARM_MODE_EVERY_DAY;
        user_alarm_add(&temp_alarm, temp_alarm.index);

        user_alarm_get_info(&temp_alarm, 2);
        temp_alarm.sw = 0;
        temp_alarm.index = 2;
        user_alarm_delete(temp_alarm.index);
        temp_alarm.mode  = E_ALARM_MODE_EVERY_DAY;
        ...
        user_alarm_get_info(&temp_alarm, 3);
        temp_alarm.sw = 0;
        temp_alarm.index = 3;
        user_alarm_delete(temp_alarm.index);
        temp_alarm.mode  = E_ALARM_MODE_ONCE;
        user_alarm_add(&temp_alarm, temp_alarm.index);

        user_alarm_get_info(&temp_alarm, 4);
        temp_alarm.sw = 0;
        temp_alarm.index = 4;
        user_alarm_delete(temp_alarm.index);
        temp_alarm.mode  = E_ALARM_MODE_ONCE;
    ...
    syscfg_read(CFG_USER_RTC_WEEK, &user_week_flag, 1);
    if((user_week_flag != 0xaa)&&(user_week_flag != 0x55))
    {
        user_week_flag = 0xaa;
    ...
    r_printf("alarm_tone_vol_1 = %d,alarm_tone_vol_2 = %d\n",alarm_tone_vol_1,alarm_tone_vol_2);
