...
static  void __fm_ui_reflash_main()//刷新主页
{
    ...
    UI_SHOW_MENU(MENU_FM_SET_FRE, 1000, __this->fm_total_channel, NULL);
...
static u16  fm_set_station_timer = 0;
static  void __user_fm_ui_show_station()//显示台号
{
    UI_SHOW_MENU(MENU_FM_SET_FRE, 500, __this->fm_total_channel, NULL);
    if(fm_set_station_timer)
    {
        sys_timeout_del(fm_set_station_timer);
    }
    fm_set_station_timer = sys_timeout_add(NULL,__fm_ui_show_station,500);
}
static u16  fm_set_fre_timer = 0;
...
static  void __user_fm_ui_cur_station(u16 timer)//显示当前台号
{
    UI_SHOW_MENU(MENU_FM_STATION, 1000, __this->fm_freq_channel_cur, NULL);\
    /* ui_menu_reflash(true); */
    if(fm_set_fre_timer)
    {
        sys_timeout_del(fm_set_fre_timer);
    }
    fm_set_fre_timer = sys_timeout_add(NULL,__fm_ui_reflash_main,timer);
    UI_MSG_POST("fm_fre", NULL);
}
/*----------------------------------------------------------------------------*/
/**@brief    fm 搜台完毕开混响
