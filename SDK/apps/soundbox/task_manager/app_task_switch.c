...
static const u8 user_app_task_list[] = {
#if TCFG_APP_BT_EN
    APP_BT_TASK,
#endif
#if TCFG_APP_MUSIC_EN
...
#endif
        break;
    case KEY_DRIVER_TYPE_AD1:
#if TCFG_ADKEY_ENABLE
        r_printf("KEY_DRIVER_TYPE_AD1\n");
        key->value += ADKEY_MAX_NUM;
        msg = adkey_event_to_msg(app_curr_task, key);
#endif
        break;
...
bool  bt_switch_next_flag = FALSE;
extern void user_ctrl_pp_network_fm(u8 cmd);
void bt_back_mode_judge(void)
{
    ...
    if(bt_back_mode_falg){
        if(!app_check_curr_task(APP_BT_TASK)){
            ...
            if(get_ble_work_state() == BLE_ST_NOTIFY_IDICATE){
                ...
                bt_back_mode_falg = FALSE;
            }
        }
    }else{
        if((get_ble_work_state() == BLE_ST_NOTIFY_IDICATE)&&app_check_curr_task(APP_BT_TASK))
        {
            bt_back_mode_falg = TRUE;
        }else{
            bt_back_mode_falg = FALSE;
    ...
    r_printf("bt_back_mode_falg %d",bt_back_mode_falg);
#endif
}
//*----------------------------------------------------------------------------*/
/**@brief    切换到下一个模式
...
void app_task_switch_next()
{
    ...
    if(!bt_back_mode_falg){
        ...
        for (cur_index = 0; cur_index < ARRAY_SIZE(user_app_task_list); cur_index++) {
            if (app_curr_task == user_app_task_list[cur_index]) {//遍历当前索引
                break;
            }
        }

        for (i = cur_index ;;) { //遍历一圈
            if (++i >= ARRAY_SIZE(user_app_task_list)) {
                i = 0;
            }
            if (i == cur_index) {
                return;
            ...
            if (app_task_switch_to(user_app_task_list[i])) {
                return;
            }
        }
    }
...
{
    if(bt_switch_next_flag){
        bt_switch_next_flag = FALSE;
    }else{
        bt_back_mode_judge();
    }
    if((app_task == APP_IDLE_TASK)||(app_task == APP_POWEROFF_TASK)){
        bt_back_mode_falg = FALSE;
    }
