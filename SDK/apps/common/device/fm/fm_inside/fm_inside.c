...
#include "asm/br23.h"
#define FMADC_LDO_VS_12v_2(x)   ((x&0x3)<<12)
void fm_init_reset()
{
    extern u32 get_chip_version();
    int val = ((get_chip_version() & 0xf) != 0x0) ? 1:2;

    JL_ANA->WLA_CON18 &= ~FMADC_LDO_VS_12v_2(0x03);
    JL_ANA->WLA_CON18 |= FMADC_LDO_VS_12v_2(val);    
}
///--------------------------FM_INSIDE_API------------------------
