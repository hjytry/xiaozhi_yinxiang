...
static u32 virfat_flash_write(void *buf, u32 addr_sec, u32 len)
{
    ...
    if (addr_sec == 0) {
        ...
        dev_bulk_read(fd, buf_temp, 0, 4096);
        dev_ioctl(fd, IOCTL_ERASE_SECTOR, 0);
        memcpy(buf_temp + real_addrsec * 512, buf, len);
        wlen = dev_bulk_write(fd, buf_temp, 0, 4096);
        len = 4096;
        ...
        dev_bulk_read(fd, buf_temp, 4 * 1024, 4096);
        dev_ioctl(fd, IOCTL_ERASE_SECTOR, 4 * 1024);
        memcpy(buf_temp + (real_addrsec * 512 - 4096), buf, len);
        wlen = dev_bulk_write(fd, buf_temp, 4096, 4096);
        len = 4096;
    } else {
        real_addrsec = addr_sec - 4096 / 512;
