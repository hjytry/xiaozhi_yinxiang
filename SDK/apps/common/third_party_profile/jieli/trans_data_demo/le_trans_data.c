...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_0000F532_1212_EFDE_1523_785FEABCD123_01_VALUE_HANDLE:

        memcpy(temp_data_buf,buffer,buffer_size);
        xz_recive_decrypt_data(temp_data_buf,buffer_size);
        extern int xz_ibuf_to_cbuf(u8 *buf, u32 len);
        int ret = xz_ibuf_to_cbuf(temp_data_buf,buffer_size);
        if(ret>0){
            
            xiao_zhi_send_even((void *)buffer_size);
        }
        break;
...
#endif
    case ATT_CHARACTERISTIC_0000F531_1212_EFDE_1523_785FEABCD123_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_ae04_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_ae05_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_ae3c_01_CLIENT_CONFIGURATION_HANDLE:
...
extern unsigned char xiao_zhi_UUID[];
...
static int make_set_adv_data(void)
{
    ...
    offset += xz_ble_gap_advertising_adv_data_update(&buf[offset]);
...
#endif
    offset += xz_ble_gap_advertising_scan_rsp_data_update(&buf[offset]);
...
void bt_ble_init(void)
{
    ...
    extern void xz_ble_app_init(void);
    extern void xz_cbuft_init(void);
    xz_cbuft_init();
    xz_ble_app_init();
...
int ex_ble_dp_data_report(u8 *p_data,u32 len)
{
    return app_send_user_data(ATT_CHARACTERISTIC_0000F531_1212_EFDE_1523_785FEABCD123_01_VALUE_HANDLE, p_data, len, ATT_OP_AUTO_READ_CCC);
}