...
static const struct adkey_platform_data *__this1 = NULL;

u8 ad_get_key_value(void);
u8 ad_get_key1_value(void);
...
struct key_driver_para adkey1_scan_para = {
    .scan_time 	  	  = 10,				//按键扫描频率, 单位: ms
    .last_key 		  = NO_KEY,  		//上一次get_value按键值, 初始化为NO_KEY;
    .filter_time  	  = 2,				//按键消抖延时;
    .long_time 		  = 75,  			//按键判定长按数量
    .hold_time 		  = (75 + 15),  	//按键判定HOLD数量
    .click_delay_time = 20,				//按键被抬起后等待连击延时数量
    .key_type		  = KEY_DRIVER_TYPE_AD1,
    .get_value 		  = ad_get_key1_value,
};
u8 ad_get_key1_value(void)
{
    u8 i;
    ...
    /* ad_data = adc_get_voltage(__this->ad_channel); */
    ad_data = adc_get_value(__this1->ad_channel);
    //r_printf("ad_value = %d \n", ad_data);
    for (i = 0; i < ADKEY1_MAX_NUM; i++) {
        if ((ad_data <= __this1->ad_value[i]) && (__this1->ad_value[i] < 0x3ffL)) {
            return __this1->key_value[i];
        }
    }
    return NO_KEY;
}
...
int adkey1_init(const struct adkey_platform_data *adkey_data)
{
    __this1 = adkey_data;
    if (!__this1) {
        return -EINVAL;
    ...
    adc_add_sample_ch(__this1->ad_channel);          //注意：初始化AD_KEY之前，先初始化ADC
#if (TCFG_ADKEY_LED_IO_REUSE || TCFG_ADKEY_IR_IO_REUSE || TCFG_ADKEY_LED_SPI_IO_REUSE)
#else
    gpio_set_die(__this1->adkey_pin, 0);
    gpio_set_direction(__this1->adkey_pin, 1);
    gpio_set_pull_down(__this1->adkey_pin, 0);
    if (__this1->extern_up_en) {
        gpio_set_pull_up(__this1->adkey_pin, 0);
    } else {
        gpio_set_pull_up(__this1->adkey_pin, 1);
    }
#endif
