...
_notify:
    ...
    extern void pwm_to_recovery_bright(void);
    extern void recovery_reduce_bright_timer(void);
    recovery_reduce_bright_timer();
    e.arg  = (void *)DEVICE_EVENT_FROM_KEY;
    r_printf("key_value: 0x%x, event: %d\n", key_value, key_event);
    extern u8 auto_reduce_bright_flag;
    if(auto_reduce_bright_flag){
    #if KEY_SLEEP_ENABLE
        if(!(((key_value == 0)&&(e.u.key.type == KEY_DRIVER_TYPE_AD1))&&((key_event == KEY_EVENT_LONG)||(key_event == KEY_EVENT_HOLD)||(key_event == KEY_EVENT_UP))))
    #endif
        {
            pwm_to_recovery_bright();
        }
        if(!(((key_value == 0)&&(e.u.key.type == KEY_DRIVER_TYPE_AD1))&&(key_event == KEY_EVENT_CLICK)))
        {
...
int key_driver_init(void)
{
    ...
    extern const struct adkey_platform_data adkey1_data;
    extern struct key_driver_para adkey1_scan_para;
    err = adkey1_init(&adkey1_data);
    if (err == 0) {

        sys_s_hi_timer_add((void *)&adkey1_scan_para, key_driver_scan, adkey1_scan_para.scan_time); //注册按键扫描定时器
    }
#endif
#endif
