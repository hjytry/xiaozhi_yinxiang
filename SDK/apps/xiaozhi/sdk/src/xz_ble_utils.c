...
void xz_ble_str_to_hex(uint8_t *str_buf,uint8_t str_len,uint8_t *hex_buf)
{
    ...
    for(j = 0 ; j < str_len ; j++) {
        data_tmp = xz_ble_char_2_ascii(str_buf[j]);
        if(data_tmp != 0xff) {
...
void xz_ble_swap(int16_t *a, int16_t *b)
{
    int16_t    temp;
    temp = *a;
    *a = *b;
...
int32_t xz_ble_hex2int(uint8_t mhex)
{
    switch(mhex) {
    case '0':
        return 0;
...
char xz_ble_hexstr2int(uint8_t *hexstr,int32_t len,uint8_t *sum)
{
    *sum = 0;
    int32_t value;
    for(int32_t i=0; i<len; i++) {
        value = xz_ble_hex2int(hexstr[i]);
        if(value == -1)return 0;
        (*sum)=(*sum)<<4;
        (*sum)+=value;
    }
...
char xz_ble_hexstr2hex(uint8_t *hexstr,int32_t len,uint8_t *hex)
{
    for(uint8_t i=0; i<len; i+=2) {
        if(xz_ble_hexstr2int(&hexstr[i],2,&hex[i/2]) == 0)
            return 0;
    }
    return 1;
}
...
void xz_ble_swap24(uint8_t dst[3], const uint8_t src[3])
{
    swapX(src, dst, 3);
}

void xz_ble_swap32(uint8_t dst[4], const uint8_t src[4])
{
    swapX(src, dst, 4);
}

void xz_ble_swap48(uint8_t dst[7], const uint8_t src[7])
{
    swapX(src, dst, 6);
}

void xz_ble_swap56(uint8_t dst[7], const uint8_t src[7])
{
    swapX(src, dst, 7);
}

void xz_ble_swap64(uint8_t dst[8], const uint8_t src[8])
{
    swapX(src, dst, 8);
}

void xz_ble_swap128(uint8_t dst[16], const uint8_t src[16])
{
    swapX(src, dst, 16);
}
...
void xz_ble_ecc_key_pem2hex_example(void)
{
	uint8_t key[64] = {0};
	uint16_t keylen = 0;
	...
	xz_ble_ecc_key_pem2hex(test_char, key, &keylen);
